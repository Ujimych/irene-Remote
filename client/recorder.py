...
class Recorder:
    ...
    def __init__(self, in_config):
        self.audio_queue = Queue(maxsize=in_config["buffersize_input"])
    ...
    def capture_callback(self, indata, frames, time, status):
        if not self.recording_flag or status: return
        audio_bytes = bytes(indata)
        try: self.audio_queue.put_nowait(audio_bytes)
        except Exception: pass

    def _create_input_stream(self):
        return sounddevice.RawInputStream(device=self.config["device"], samplerate=self.config["samplerate"], blocksize=self.config["blocksize"], dtype=self.config["dtype"], channels=self.config["channels"], callback=self.capture_callback)

    async def producer(self):
        self._logger.debug("Начинаем захват аудиоданных.")
        try:
    ...
    async def consumer(self, in_websocket):
        while True:
            ...
            try:
                ...
                self._logger.error(f"Ошибка отправки данных: {e}")
